...
export class DataService {
  readonly PRINTS: Print[] = [
    ...
    },
    {
      ...
      community: true,
      versions: [
        { tag: '1.0.0', date: '2026-04-02', notes: 'Initial release.' },
      ],
    ...
    },
    {
      ...
      versions: [
        { tag: '0.1.0', date: '2026-01-28', notes: 'Initial release.' },
      ],
  ...
  readonly TEAM_PICKS = ['samanthax/kickstart-fantasy-book', 'amine-barra/aws-dva-c02-practice', 'sterixxx/dnd-gm'];

  readonly MOCK_ACTIVITY: ActivityItem[] = [
  ...
  readonly MOCK_STARRED: Print[] = this.PRINTS.slice(1, 4);

  readonly MOCK_FILES: Record<string, string> = {
  ...
  readonly DIFF_EXAMPLE = {
    file: 'identities/roles.yaml',
    v1: '1.0.0',
    v2: '1.3.0',
    left: this.MOCK_FILES['identities/roles_v1.yaml'],
    right: this.MOCK_FILES['identities/roles.yaml'],
  };

  getPrintById(id: string): Print | undefined {
    return this.PRINTS.find(p => p.id === id);
  }

  getTeamPickPrints(): Print[] {
    return this.TEAM_PICKS.map(id => this.getPrintById(id)).filter((p): p is Print => !!p);
  }
  ...
  getLang(filename: string): string {
    ...
    const map: Record<string, string> = {
      ...
      yaml: 'yaml', yml: 'yaml', md: 'markdown', prisma: 'css',
      json: 'json', toml: 'toml', adoc: 'markup',
    };
