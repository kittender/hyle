...
import { readLock, upsertLockEntry, computeFileChecksums } from "../lock";
...
export async function runUpgrade(name: string | undefined, opts: UpgradeOptions): Promise<void> {
    ...
    for (const entry of toUpgrade) {
      try {
        ...
        if (!gt(latest.version, entry.version)) {
          if (process.stdin.isTTY !== false) {
            console.log(`⊘ ${entry.author}/${entry.name}: already up-to-date (${entry.version})`);
          }
          skipped++;
          continue;
        }
        ...
        // Check dependencies
        if (latest.manifest.dependencies && latest.manifest.dependencies.length > 0) {
          const missingDeps = await checkMissingDeps(latest.manifest.dependencies);
        ...
        console.error(`✗ ${entry.author}/${entry.name}: ${(e as Error).message}`);
      }
    }
