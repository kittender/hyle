...
interface VerifyResult {
  entry: string;
  files: Array<{ path: string; ok: boolean; expected?: string; actual?: string }>;
      ...
      for (const result of results) {
          ...
          for (const file of result.files) {
            if (!file.ok) {
              ...
              if (file.actual && file.actual !== "NOT_FOUND") {
                console.log(`    Expected: ${file.expected?.substring(0, 8)}...`);
        ...
        if (result.versionStatus) {
          if (result.versionStatus === "outdated") {
            console.log(`  ⬆️ Version is outdated (use 'hyle upgrade' to update)`);
