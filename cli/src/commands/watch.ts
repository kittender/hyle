import { readFileSync, writeFileSync, appendFileSync, existsSync, statSync, readdirSync, openSync, readSync } from "node:fs";
...
export async function runWatch(opts: WatchOptions): Promise<void> {
  ...
  try {
    ...
    const pollInterval = setInterval(() => {
      const newTurns = pollJSONLByLineCount(session.jsonlPath, lastProcessedLineCount);
      ...
      try {
        ...
        if (contentCwdNormalized === cwdNormalized && content.sessionId) {
          ...
          for (const projectDir of projectDirs) {
            const jsonlPath = join(projectsBase, projectDir, `${content.sessionId}.jsonl`);
...
function readBaseline(jsonlPath: string): { turns: TurnStats[]; lineCount: number } {
  const turns: TurnStats[] = [];
  ...
  try {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (record.type === "assistant" && record.message?.usage && record.requestId) {
...
function pollJSONLByLineCount(jsonlPath: string, lastProcessedLineCount: number): TurnStats[] {
  const turns: TurnStats[] = [];
...
function extractTurnStats(record: any, seq: number): TurnStats {
  ...
  const timestamp = new Date(record.timestamp || Date.now()).toLocaleTimeString();
...
function renderUI(turns: TurnStats[], session: SessionInfo): void {
  ...
  // Header
  console.log(`hyle watch — session: ${session.sessionId}  model: claude-${model}  ${session.gitBranch ? `branch: ${session.gitBranch}` : ""}`);
  ...
  if (contextPercent >= 80) color = "\x1b[31m"; // Red
  else if (contextPercent >= 60) color = "\x1b[33m"; // Yellow

  console.log(`Context: ${color}${bar}\x1b[0m  ${contextPercent}%  (${contextUsed.toLocaleString()} / ${contextWindow.toLocaleString()} tokens)`);
  ...
  for (const turn of turns.slice(-20)) {
    ...
    console.log(` #${String(turn.seq).padStart(2)}   ${turn.timestamp}   ${input}   ${output}   ${cacheR}   ${cacheW}`);
  }
  ...
  console.log(` TOTAL                ${tInput}   ${tOutput}   ${tCacheR}   ${tCacheW}`);

  console.log("\nWatching for new turns... Ctrl+C to exit");
}

function parseThreshold(threshold: string): { type: "percent" | "absolute"; value: number } {
  if (threshold.endsWith("%")) {
    return { type: "percent", value: parseInt(threshold) };
  }
  return { type: "absolute", value: parseInt(threshold) };
}

function shouldSplit(turns: TurnStats[], threshold: { type: "percent" | "absolute"; value: number }, session: SessionInfo): boolean {
  const totalInput = turns.reduce((s, t) => s + t.inputTokens, 0);
  const totalCacheRead = turns.reduce((s, t) => s + t.cacheReadTokens, 0);
  const contextUsed = totalInput + totalCacheRead;
...
function writeCatchupFile(cwd: string, session: SessionInfo, turns: TurnStats[]): string {
...
${turns.slice(-3).map(t => `- (Turn ${t.seq}) Input: ${t.inputTokens}, Output: ${t.outputTokens}`).join("\n")}
...
function showSplitWarning(catchupPath: string): void {
  ...
  console.log(`OSC link: \x1b]8;;file://${catchupPath}\x1b\\[ open catch-up ]\x1b]8;;\x1b\\\n`);
}

function appendAuditEntry(cwd: string, turn: TurnStats, state: AuditState): AuditState {
  const auditPath = join(cwd, "hyle-audit.log");
